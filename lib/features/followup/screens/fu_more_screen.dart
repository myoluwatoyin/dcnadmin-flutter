...
class FuMoreScreen extends ConsumerWidget {
  const FuMoreScreen({super.key, this.onOpenTab});
      ...
      child: Column(
        ...
        children: [
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(user?.fullName ?? 'Worker', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800, color: c.text)),
                        const SizedBox(height: 2),
                        Text([user?.departmentName, user?.subUnitName].where((s) => (s ?? '').isNotEmpty).join(' · '), style: TextStyle(fontSize: 12.5, color: c.textMuted)),
                        const SizedBox(height: 8),
                        Wrap(spacing: 6, children: [
                          const DcnChip(label: 'Follow-up worker', tone: DcnTone.brand, size: DcnButtonSize.sm),
          ...
          _Group(title: 'Ministry', rows: [
            (icon: 'plus', title: 'Add member', sub: 'Manual, bulk, or auto-sync', onTap: () => context.push('/fu/add-member')),
            (icon: 'users', title: 'Follow-up queue', sub: "This week's follow-ups", onTap: () => context.push('/fu/queue')),
            ...
            (icon: 'send', title: 'Send SMS', sub: null, onTap: () => context.push('/fu/sms')),
            (icon: 'award', title: 'Scorecards', sub: null, onTap: () => context.push('/scorecards')),
            (icon: 'users', title: 'Accountability partner', sub: null, onTap: () => context.push('/accountability')),
          ]),
